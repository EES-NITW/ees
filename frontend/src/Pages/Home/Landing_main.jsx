import Particles from "../../components/Particles";
import Navbar from "../../components/Navbar";
import "./Landing_main.css";
import DeptImage from "../../assets/eee-dept.png";
import gensecImg from "../../assets/Photos/Gensec.jpg";
import Sec1 from "../../assets/Photos/Sec1.png";
import Sec2 from "../../assets/Photos/Sec2.png";
import grouppic from "../../assets/Photos/group.jpeg";
import {
  FaFacebookF,
  FaInstagram,
  FaLinkedin,
  FaArrowRight,
  FaCalendarAlt,
  FaClock,
  FaMapMarkerAlt,
} from "react-icons/fa";
import Snowfall from 'react-snowfall';

const Landing2 = () => {
  return (
    <div className="w-full min-h-screen relative bg-black">
      <Snowfall
        style={{
          position: "fixed",
          width: "100vw",
          height: "100vh",
        }}
        color="#82c3d9"
      />
      <div className="header">
        <p className="text-blue-300 tracking-wide text-sm mb-2 mt-6">
          EST. 2002
        </p>
        <div className="gap-0">
          <h1 className="text-white font-bold leading-tight text-4xl sm:text-4xl md:text-6xl lg:text-7xl">
            Powering Innovation,
          </h1>
          <h2 className="font-bold text-white text-4xl sm:text-5xl md:text-6xl lg:text-7xl">
            Energizing Future
          </h2>
        </div>
        <p className="mt-6 mb-6 text-gray-400 font-bold text-sm sm:text-base md:text-4xl">
          Then, Now and Beyond.
        </p>
        <p className="max-w-2xl text-gray-400 text-lg md:text-xl mb-10 ">
          The Electrical Engineering Society at NITW fosters
          innovation,collaboration, and technical excellence - bridging theory
          with real-world impact.
        </p>
        <a
          href="https://www.nitw.ac.in/eee"
          target="_blank"
          rel="noopener noreferrer"
          className="flex items-center gap-2 px-8 py-3 bg-blue-500 text-white rounded-md font-medium hover:bg-blue-600 transition border-blue-500 border-2"
        >
          <span>NITW EEE</span>
          <FaArrowRight />
        </a>
      </div>

      <div className="content">
        <div className="relative w-full bg-black py-6 px-4 md:px-10 h-screen">
          <div className="max-w-5xl mx-auto bg-gradient-to-br from-gray-900 to-black rounded-3xl shadow-xl border border-gray-700/60 p-6 md:p-10 flex flex-col md:flex-row gap-8 items-center">
            <div className="w-full md:w-1/2">
              <img
                src={DeptImage}
                alt="Electrical Engineering Department"
                className="w-full h-full object-cover rounded-2xl"
              />
            </div>
            <div className="w-full md:w-1/2 text-gray-200 text-base md:text-lg leading-relaxed text-center">
              <p className="mb-4 font-serif text-[22px]">
                The Electrical Engineering Department is one of the oldest
                departments of the National Institute of Technology, Warangal.
              </p>
              <p className="mb-4 font-serif text-[22px]">
                Since its inception, the department has actively engaged in
                teaching and research across diverse areas of Electrical
                Engineering, supported by experienced faculty and strong
                laboratories.
              </p>
            </div>
          </div>
        </div>
        <div className="relative w-full bg-black py-6 px-4 md:px-10 ">
          <div className="max-w-5xl mx-auto bg-gradient-to-br from-gray-900 to-black rounded-3xl shadow-xl border border-gray-700/60 p-6 md:p-10 gap-8 items-center">
            <div className="w-full text-gray-200 text-base md:text-lg leading-relaxed text-center">
              <p className="mb-4 font-serif text-[22px]">
                The Electrical Engineering Society (EES) acts as a platform
                where students can explore their interests, collaborate with
                peers, and showcase their talents. The society organizes a range
                of events throughout the year, from technical workshops to
                cultural events and sports tournaments, ensuring a balance
                between academic and extracurricular pursuits. EES also plays a
                pivotal role in bridging the gap between academics and industry.
                Guest lectures, industrial visits, and alumni interactions
                provide students with insights into real-world applications of
                their studies, inspiring innovation and career growth.
              </p>
            </div>
            <div className="w-full  h-95">
              <img
                src={grouppic}
                alt="Electrical Engineering Department"
                className="w-full h-full object-cover rounded-2xl"
              />
            </div>
          </div>
        </div>
        <div className="max-w-5xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-15 text-center mb-20 mt-10">
          <div class="w-full max-w-sm rounded-2xl bg-gradient-to-br from-[#0b1220] to-[#0a0f1a] p-8 cursor-pointer border-2 border-transparent transform transition-all duration-300 ease-out hover:-translate-y-3 hover:border-blue-300 hover:shadow-2x hover:shadow-blue-500/20">
            <h2 class="text-white text-5xl font-bold leading-none">250+</h2>
            <p className="mt-2 text-sm md:text-base text-gray-300 font-mont font-semibold">
              Active Members
            </p>
          </div>
          <div class="w-full max-w-sm rounded-2xl bg-gradient-to-br from-[#0b1220] to-[#0a0f1a] p-8 cursor-pointer border-2 border-transparent transform transition-all duration-300 ease-out hover:-translate-y-3 hover:border-blue-300 hover:shadow-2x hover:shadow-blue-500/20">
            <h2 class="text-white text-5xl font-bold leading-none">50+</h2>
            <p className="mt-2 text-sm md:text-base text-gray-300 font-mont font-semibold">
              Events Hosted
            </p>
          </div>
          <div class="w-full max-w-sm rounded-2xl bg-gradient-to-br from-[#0b1220] to-[#0a0f1a] p-8 cursor-pointer border-2 border-transparent transform transition-all duration-300 ease-out hover:-translate-y-3 hover:border-blue-300 hover:shadow-2x hower:border-sm hover:shadow-blue-500/20">
            <h2 class="text-white text-5xl font-bold leading-none">25+</h2>
            <p className="mt-2 text-sm md:text-base text-gray-300 font-mont font-semibold">
              Workshops
            </p>
          </div>
          <div class="w-full max-w-sm rounded-2xl bg-gradient-to-br from-[#0b1220] to-[#0a0f1a] p-8 cursor-pointer border-2 border-transparent transform transition-all duration-300 ease-out hover:-translate-y-3 hover:border-blue-300 hover:shadow-2x hover:shadow-blue-500/20">
            <h2 class="text-white text-5xl font-bold leading-none">90%+</h2>
            <p className="mt-2 text-sm md:text-base text-gray-300 font-mont font-semibold">
              Placement Rate
            </p>
          </div>
        </div>

        <div className="mb-20 mx-10 mt-10">
          <h1 className="text-white text-center font-bold text-4xl mb-10">
            What We Do
          </h1>
          <div className="flex flex-row items-center justify-evenly mb-10">
            <div class="w-full max-w-sm rounded-2xl bg-gradient-to-br from-[#0b1220] to-[#0a0f1a] p-8 cursor-pointer border-2 border-gray-800 transform transition-all duration-300 ease-out hover:-translate-y-3 hover:border-blue-300 hover:shadow-2x hover:shadow-blue-500/20 ">
              <h2 className="font-bold text-2xl mb-3">Industry Connect</h2>
              <p className="text-gray-400">
                Guest lectures from leading companies, industrial visits, and
                networking opportunities
              </p>
            </div>
            <div class="w-full max-w-sm rounded-2xl bg-gradient-to-br from-[#0b1220] to-[#0a0f1a] p-8 cursor-pointer border-2 border-gray-800 transform transition-all duration-300 ease-out hover:-translate-y-3 hover:border-blue-300 hover:shadow-2x hover:shadow-blue-500/20 ">
              <h2 className="font-bold text-2xl mb-3">Community Building</h2>
              <p className="text-gray-400">
                Alumni network, peer mentoring, social events, and collaborative
                learning environment
              </p>
            </div>
          </div>
          <div className="flex flex-row justify-evenly">
            <div class="w-full max-w-sm rounded-2xl bg-gradient-to-br from-[#0b1220] to-[#0a0f1a] p-8 cursor-pointer border-2 border-gray-800 transform transition-all duration-300 ease-out hover:-translate-y-3 hover:border-blue-300 hover:shadow-2x hover:shadow-blue-500/20 ">
              <h2 className="font-bold text-2xl mb-3">Skill Development</h2>
              <p className="text-gray-400">
                Placement preparation, technical certifications, mock
                interviews, and aptitude training
              </p>
            </div>
            <div class="w-full max-w-sm rounded-2xl bg-gradient-to-br from-[#0b1220] to-[#0a0f1a] p-8 cursor-pointer border-2 border-gray-800 transform transition-all duration-300 ease-out hover:-translate-y-3 hover:border-blue-300 hover:shadow-2x hover:shadow-blue-500/20 ">
              <h2 className="font-bold text-2xl mb-3">Competitions</h2>
              <p className="text-gray-400">
                Organize and participate in Innothon, tech fests, and
                inter-college competitions
              </p>
            </div>
          </div>
        </div>

        <div className="mb-20 mx-10 mt-25">
          <h1 className="text-white text-center font-bold text-4xl mb-10">
            Upcoming Events
          </h1>
          <div className="flex justify-center mb-10">
            <div className="group w-1/2 rounded-2xl bg-gradient-to-br from-[#0b1220] to-[#0a0f1a] p-8 cursor-pointer border-2 border-gray-800 transform hover:border-blue-300 hover:shadow-2x hover:shadow-blue-500/20 ">
              <h2 className="font-bold text-2xl mb-3 text-white group-hover:text-blue-400 transition-colors">
                Power and Energy Research
              </h2>
              <p className="text-gray-400 mb-3">
                Guest lecture on Power and Energy Research : A path for future
                Engineers
              </p>
              <p
                style={{ display: "flex", alignItems: "center" }}
                className="text-gray-400 mb-2"
              >
                <FaCalendarAlt
                  style={{ marginRight: "8px", color: "#4da3ff" }}
                />
                January 3,2026
              </p>
              <p
                style={{ display: "flex", alignItems: "center" }}
                className="text-gray-400 mb-2 "
              >
                <FaClock style={{ marginRight: "8px", color: "#4da3ff" }} />
                5:00 PM
              </p>
              <p
                style={{ display: "flex", alignItems: "center" }}
                className="text-gray-400 mb-4"
              >
                <FaMapMarkerAlt
                  style={{ marginRight: "8px", color: "#4da3ff" }}
                />
                EED Seminar Hall
              </p>
              <p className="text-gray-400 ">Speaker : Dr.Ankit Singhal</p>
            </div>
          </div>
          <div className="flex justify-center">
            <a
              href="/Events/Events.jsx"
              target="_blank"
              rel="noopener noreferrer"
              className="border-2 flex  items-center gap-2 border-blue-400 text-blue-300 rounded-md py-2 px-4 hover:text-white hover:border-blue-600 transition-all duration-300 hover:cursor-pointer"
            >
              <span>View All Events</span>
              <FaArrowRight />
            </a>
          </div>
        </div>

        <div className="mb-20 mx-10 my-25 ">
          <h1 className="text-white text-center font-bold text-4xl mb-10">
            Meet The Team
          </h1>
          <div className="flex justify-around gap-5 items-strech mb-10">
            <div className="rounded-2xl bg-gradient-to-br from-[#0b1220] to-[#0a0f1a] pb-4 border-2 border-gray-800 cursor-pointer text-center transform transition-all duration-300 ease-out hover:-translate-y-3 hover:-translate-y-3 hover:border-blue-300 hover:shadow-2x hover:shadow-blue-500/20  w-[28%] ">
              <div className="flex justify-center items-center">
                <img
                  src={Sec1}
                  alt="Secretary"
                  className="h-48 w-48 mb-4 rounded-md mt-5 object-cover"
                ></img>
              </div>
              <h2 className="font-bold text-2xl mb-3 text-white  group-hover:text-blue-400 transition-colors">
                Kammula Keerti
              </h2>
              <p className="text-blue-400 mb-3 font-mont font-semibold">
                Secretary
              </p>
              <p className="text-gray-400 mb-3">EEE - Final Year</p>
            </div>
            <div className="rounded-2xl bg-gradient-to-br from-[#0b1220] to-[#0a0f1a] pb-4 border-2 border-gray-800 cursor-pointer text-center transform transition-all duration-300 ease-out hover:-translate-y-3 hover:-translate-y-3 hover:border-blue-300 hover:shadow-2x hover:shadow-blue-500/20  w-[28%] ">
              <div className="flex justify-center items-center">
                <img
                  src={gensecImg}
                  alt="General Secretary"
                  className="h-45 w-45 mb-4 rounded-md mt-5 object-cover"
                ></img>
              </div>
              <h2 className="font-bold text-2xl mb-3 text-white group-hover:text-blue-400 transition-colors">
                Pulipati Goutham Datta
              </h2>
              <p className="text-blue-400 mb-3 font-mont font-semibold">
                General Secretary
              </p>
              <p className="text-gray-400 mb-3">EEE - Final Year</p>
            </div>
            <div className="rounded-2xl bg-gradient-to-br from-[#0b1220] to-[#0a0f1a] pb-4 border-2 border-gray-800 cursor-pointer text-center transform transition-all duration-300 ease-out hover:-translate-y-3 hover:-translate-y-3 hover:border-blue-300 hover:shadow-2x hover:shadow-blue-500/20  w-[28%] ">
              <div className="flex justify-center items-center">
                <img
                  src={Sec2}
                  alt="Secretary"
                  className="h-48 w-48 mb-4 rounded-md mt-5 object-cover"
                ></img>
              </div>
              <h2 className="font-bold text-2xl mb-3 text-white group-hover:text-blue-400 transition-colors">
                Bandaru Teja
              </h2>
              <p className="text-blue-400 mb-3 font-mont font-semibold">
                Secretary
              </p>
              <p className="text-gray-400 mb-3">EEE - Final Year</p>
            </div>
          </div>
          <div className="flex justify-center">
            <a
              href="/Teams/Teams.jsx"
              target="_blank"
              rel="noopener noreferrer"
              className="border-2 flex items-center gap-2 border-blue-400 text-blue-300 rounded-md py-2 px-4 hover:text-white hover:border-blue-600 transition-all duration-300 hover:cursor-pointer"
            >
              <span>View Full Team</span>
              <FaArrowRight />
            </a>
          </div>
        </div>
      </div>

      <div className="footer">
        <div className="footer1">
          <div>
            <img
              src="data:image/png;base64,"
              alt="ees_logo"
              className="footer-logo"
            ></img>
          </div>
          <div className="footer-cont">
            <h2 className="footer-text">EES | NITW</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-10 text-gray-400 ">
              <div>
                <h3 className="text-white font-semibold mb-4">Quick Links</h3>
                <ul className="space-y-1">
                  <li>
                    <a href="/events" className="hover:text-white transition ">
                      Events
                    </a>
                  </li>
                  <li>
                    <a href="/teams" className="hover:text-white transition">
                      Teams
                    </a>
                  </li>
                  <li>
                    <a href="/databank" className="hover:text-white transition">
                      Databank
                    </a>
                  </li>
                  <li>
                    <a
                      href="/placements"
                      className="hover:text-white transition"
                    >
                      Placements
                    </a>
                  </li>
                </ul>
              </div>
              <div>
                <h3 className="text-white font-semibold mb-4">Contact</h3>
                <ul className="space-y-3">
                  <li>
                    Email:{" "}
                    <span className="text-gray-400">eesociety@nitw.ac.in</span>
                  </li>
                  <li>NITW Campus, Warangal</li>
                </ul>
              </div>
              <div>
                <h3 className="text-white font-semibold mb-4">Follow Us</h3>
                <p className="mb-4">Stay connected on social media</p>

                <div className="flex gap-6">
                  <a
                    href="https://www.facebook.com/eeanitw/"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-11 h-11 flex items-center justify-center border border-gray-400 rounded-full text-white hover:text-blue-600 hover:border-blue-600 transition-colors"
                  >
                    <FaFacebookF size={18} />
                  </a>
                  <a
                    href="https://www.instagram.com/nitw_ees/"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-11 h-11 flex items-center justify-center border border-gray-400 rounded-full text-white hover:text-pink-500 hover:border-pink-500 transition-colors"
                  >
                    <FaInstagram size={18} />
                  </a>
                  <a
                    href="https://www.linkedin.com/company/electrical-engineering-society-ees-nit-warangal/"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-11 h-11 flex items-center justify-center border border-gray-400 rounded-full text-white hover:text-blue-600 hover:border-blue-600 transition-colors"
                  >
                    <FaLinkedin size={18} />
                  </a>
                </div>
              </div>
            </div>
          </div>
          <div>
            <img
              src="data:image/png;base64,"
              alt="NITW_logo"
              className="footer-logo"
            ></img>
          </div>
        </div>
        <hr className="border-t border-gray-500/40 pb-8 w-[95%]" />
        <div className="footer2 text-gray-400">
          <p>
            © Electrical Engineering Society, NIT Warangal. All rights reserved.
          </p>
        </div>
      </div>
    </div>
  );
};

export default Landing2;
